...
std::string     RPL_JOIN(Client cl, Channel channel, String topic) {
    if (channel.get_file_descriptorOp() == cl.get_file_descriptor())
...
void        join(Channel &chan, Client &cl)
{
    ...
    {
        if (chan.get_Clients()[i].get_file_descriptor() == chan.get_file_descriptorOp())
...
int         Server::join_Command(std::vector<String> params, Client &cl)
{
