...
String RPL_PRIVMSG(Client cl, String recipient, String message)
{
    ...
    return (":" + cl.get_Nick_name() + " PRIVMSG " + recipient + " :" + message);
}
...
int Server::change_Message(std::vector<String> params, Client &cl)
{
    if (params.size() < 3)
    {
    ...
    try
    {
        ...
        if (isClientInChannel(*chan, cl.get_file_descriptor()))
            chan->broadcast_message(RPL_PRIVMSG(cl, params[1], message), cl);
        else
...
int Server::private_message_Command(std::vector<String> params, Client &cl)
{
    ...
    if (params.size() < 3)
	{
        cl.reply("461 " + cl.get_Nick_name() + " " + "PRIVMSG" + " :Not enough parameters");
		return -1;
	}
    if (params[1].at(0) == '#')
        return (change_Message(params, cl));
    try
    {
        Client  recipient = findClient(params[1]);      
        String  msg = getMessage(params);
        std::cout << "message = " << "[" << msg << "]" << std::endl;
        String paquet = RPL_PRIVMSG(cl, recipient.get_Nick_name(), msg);
